...
- 接著是數個主題式章節，每個都用「## 主題：一句話副標」的形式下標題（例如「核心命盤：一個以「付出」為底色的靈魂」、「事業與茁壯：選擇艱辛而光榮的那條路」、「生命功課：從壓抑到真正的堅毅」）。建議的章節切分方式：
  1. 核心命盤（太陽・本命循環）
  2. 情緒與形象（月亮・情緒的安全感 + 上升・特質＆形象）
  3. 頭腦與天賦（水星・學習＆傳達 + 天王星・創意＆自由）
  4. 情感與行動（金星・戀情＆人際關係 + 火星・行動＆創造）
  5. 生命格局（木星）
  6. 傳統與制約（土星）
  7. 愛與藝術（海王星）
  8. 靈性發展（冥王星）
  9. 靈魂的方向（南交・節制＆特質 → 北交・良善＆美德）
- 一條水平分隔線（三個減號）
...
【寫作風格】
- 資料最後附有「元素層參考資料」：四張 Ace 把 22 張大牌分成火（榮譽與地位）、水（愛與關係）、風（情仇與思緒）、土（物質與享受）四族，每張牌只屬於一個元素，並附有該元素的正面與負面描述。請務必解讀這張命盤的**元素比重**——哪個元素特別多、哪個特別少或缺席，代表這個人的生命重心與相對弱項；可以獨立成一段，或融入核心命盤那一段。同樣不得超出資料範圍杜撰。
...
function buildUserPrompt({ birth, numbers, gods }) {
  ...
  const balance = elementBalance(numbers, elements);
  const lines = [];
  ...
  lines.push("");
  lines.push(`元素比重（13 個位置的分布）：${Object.entries(balance).map(([el, n]) => `${el} ${n}`).join("　")}`);
  lines.push("");
  ...
  for (const planet of PLANETS) {
    ...
    const element = elementOf(value, elements);
    if (element) {
      lines.push(`（元素｜${element.element}・${element.ace}：${element.title}）`);
    }
  ...
  lines.push("");
  lines.push("===== 元素層參考資料（四張 Ace，供解讀元素比重時取用）=====");
  lines.push("");
  lines.push(fs.readFileSync(path.join(__dirname, "..", "content", "elements.md"), "utf8").split("\n## 備註")[0]);
...
module.exports = { buildUserPrompt, SYSTEM_PROMPT };
